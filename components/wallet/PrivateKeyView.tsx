    ...
    <div className="flex size-full flex-col gap-6 px-6 py-5 max-standard:min-h-screen">
      ...
      <div className="flex h-full w-full flex-col gap-4">
        ...
        <div className="flex items-center gap-2.5 rounded-xl border p-4 font-medium text-white/75 text-xs border-yellow-200 bg-yellow-200/10 [&_svg]:text-yellow-200">
          ...
          Please ensure you have securely backed up your private key. You will need it to restore your wallet.
        </div>

        {/* Private Key Display */}
        <div className="w-full">
          <div 
            className="relative cursor-pointer break-all rounded-xl border border-white/14 bg-white/7 px-6 py-4 font-medium text-neutral-200 text-sm transition-colors hover:bg-neutral-700 flex items-center gap-2"
            onClick={() => {
              if (privateKey) {
          ...
          >
            <span className="flex-1">{privateKey}</span>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" className="inline size-4 pl-1">
