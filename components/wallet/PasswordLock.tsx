...
interface PasswordLockProps {
  ...
  /** Show BIP39 passphrase field (envelope metadata while locked). */
...
export const PasswordLock: React.FC<PasswordLockProps> = ({
  ...
  const handleUnlock = async (e: React.FormEvent) => {
    ...
    if (passphraseRequired && !passphrase.trim()) {
      setErrorMessage('Enter your BIP39 passphrase.');
      return;
    }

    try {
      await onUnlock(password, passphraseRequired ? passphrase : undefined);
      setPassword('');
      setPassphrase('');
    } catch (err) {
      setErrorMessage(err instanceof Error ? err.message : 'Unlock failed');
      setPassword('');
      setPassphrase('');
      if (inputRef.current) {
        inputRef.current.focus();
  ...
  const handleSetPassword = async (e: React.FormEvent) => {
    ...
    const policySet = evaluateNewPassword(password);
    if (!policySet.ok) {
      setErrorMessage(policySet.error);
      return;
    }
  ...
  return (
    <div className="rounded-xl border border-white/7 bg-neutral-800 p-6">
      ...
      <form onSubmit={isInitialPasswordMode ? handleInitialPassword : (isSettingPassword ? handleSetPassword : handleUnlock)} className="space-y-4">
        ...
        {!isSettingPassword && !isInitialPasswordMode && passphraseRequired && (
          <div>
            <label htmlFor="bip39UnlockPassphrase" className="block text-sm font-medium text-neutral-200 mb-2">
              BIP39 passphrase
            </label>
            <p className="text-xs text-neutral-400 mb-2">
              Not your vault password. Required every unlock for this wallet.
            </p>
            <div className="flex w-full flex-col justify-center gap-2 rounded-xl border border-white/7 bg-neutral-700 transition-colors focus-within:bg-neutral-600 focus-within:border-white/7 h-12 px-5">
              <div className="flex w-full items-center justify-between gap-2">
                <input
                  id="bip39UnlockPassphrase"
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center bg-transparent m-0 p-0"
                >
        ...
        <p className="text-xs text-neutral-400 mt-4">
          {isInitialPasswordMode || isSettingPassword
            ...
            : 'Enter your password to access the wallet. The wallet will lock after 5 minutes of inactivity.'}
        </p>
      </form>
    </div>
  );
